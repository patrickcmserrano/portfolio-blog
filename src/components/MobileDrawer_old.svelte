<script lang="ts">
	import { Drawe		<nav class="flex flex-row justify-end space-x-4 p-4">
...
<div class="fixed bottom-0 left-0 right-0 z-10 sm:hidden">
	...
	>
		...
		<nav class="flex flex-row justify-end space-x-4 p-4">
			<NavLink href="{base}/" label={$_('nav.home')} on:click={() => drawerStore.close()} />
			<NavLink href="{base}/about" label={$_('nav.about')} on:click={() => drawerStore.close()} />
			<NavLink href="{base}/blog" label={$_('nav.blog')} on:click={() => drawerStore.close()} />
			<NavLink
				href="{base}/about-site"
				label={$_('nav.aboutSite')}
				on:click={() => drawerStore.close()}
			/>
		</nav>
	</Drawer>
